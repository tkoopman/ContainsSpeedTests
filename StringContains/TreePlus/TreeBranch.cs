...
namespace StringContains.TreePlus
{
    public sealed class TreeBranch (int depth)
    {
        ...
        {
            if (Branches.Count == 0)
                return new FrozenTreeBranch(-1, 0, Leafs.ToFrozenSet(), null);
            ...
            if (anyLeafs || keySize == -1)
                ...
                return new FrozenTreeBranch(1, 1, Leafs.Count == 0 ? null : Leafs.ToFrozenSet(), branches.ToFrozenDictionary());
            ...
            foreach (var (key, branch) in branches)
            {
                foreach (var (key2, branch2) in branch.Branches!)
                {
                    ...
                    minLen = int.Min(minLen, branch2.MinLen);
                }
            }

            return new FrozenTreeBranch(keySize, Leafs.Count == 0 ? minLen : keySize, Leafs.Count == 0 ? null : Leafs.ToFrozenSet(), newBranches.ToFrozenDictionary());
        }
    }
}